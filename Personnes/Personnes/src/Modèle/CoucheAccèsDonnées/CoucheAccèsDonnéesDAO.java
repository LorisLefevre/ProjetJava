...
public class CoucheAccèsDonnéesDAO implements CoucheAccèsDonnées
{
    ...
    @Override
    public boolean emprunterLivreStream(int id) {
        List<Livre> livres = chargerLivres();
        Livre livreEmprunté = null;

        // Recherche du livre avec l'ID donné
        ...
        if (livreEmprunté != null) {
            ...
            livreEmprunté.setEmprunter(true);
            // Sauvegarder les modifications dans le fichier
            sauvegarderLivres(livres);
            System.out.println("Livre emprunté avec succès : " + livreEmprunté);
            return true;
        } else {
            System.out.println("Livre non trouvé avec l'ID : " + id);
            return false;
    ...
    private List<Livre> chargerLivres() {
        List<Livre> livres = new ArrayList<>();
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(LIVRES_FILE_PATH))) {
            while (true) {
                Livre livre = (Livre) ois.readObject();
                livres.add(livre);
            }
        } catch (EOFException e) {
            // Fin de fichier atteinte
        } catch (IOException | ClassNotFoundException e) {
            System.err.println("Erreur lors de la lecture du fichier de livres : " + e.getMessage());
        }
        return livres;
    }

    private void sauvegarderLivres(List<Livre> livres) {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(LIVRES_FILE_PATH))) {
            for (Livre livre : livres) {
                oos.writeObject(livre);
            }
        } catch (IOException e) {
            System.err.println("Erreur lors de l'écriture dans le fichier de livres : " + e.getMessage());
        }
    }
    ...
    private static final String LIVRES_FILE_PATH = "livres.dat";


    @Override
    public void ajouterLivre(Livre livre)
    {
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(LIVRES_FILE_PATH, true)))
        {
            oos.writeObject(livre);
            System.out.println("Livre ajouté avec succès : " + livre);
        }
        catch (IOException e) {
            System.err.println("Erreur lors de l'écriture du fichier de livres : " + e.getMessage());
        }
    }
    @Override
    ...
    {
        // Créer une liste pour stocker les livres
        List<Livre> livres = new ArrayList<>();

        // Lire tous les livres depuis le fichier
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(LIVRES_FILE_PATH))) {
            while (true) {
                Livre livre = (Livre) ois.readObject();
                // Vérifier si l'ID du livre correspond à celui à supprimer
                if (livre.getID() != id) {
        ...
        // Écrire à nouveau tous les livres, sauf celui qui a été supprimé
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream(LIVRES_FILE_PATH))) {
            for (Livre livre : livres) {
                oos.writeObject(livre);
            }
